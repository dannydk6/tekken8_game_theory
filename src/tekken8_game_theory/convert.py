...
"""
    ...
    input_dict['P1-strats-str'] = ' '.join(f'"{strat}"' for strat in input_dict["P1-strats"])
    input_dict['P2-strats-str'] = ' '.join(f'"{strat}"' for strat in input_dict["P2-strats"])
    payoff_list, indices = get_gambit_matrix(input_dict["payoffs"])
    input_dict['payoff-list-str'] = '\n'.join(f'{{ "{payoff[0]}" {payoff[1]}, {payoff[2]} }}' for payoff in payoff_list)
